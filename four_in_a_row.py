...
def getPlayerMove(playerTile, board):
    ...
    while True:  # Keep asking player until they enter a valid move.
        ...
        if reponse not in COLUMN_LABELS:
            print(f"Enter a number from 1 to {BOARD_WIDTH}.")
            continue  # Ask player again for their move.

        columnIndex = int(reponse) - 1  # -1 for 0-based column indexes.

        # If the column is full, ask for a move again:
        if board[(columnIndex, 0)] != EMPTY_SPACE:
            print("That column if full, ask for a move again:")
...
def isFull(board):
    """Returns True uf the 'board' has no empty spaces, otherwise return False"""
    for rowIndex in range(BOARD_HEIGHT):
        for columnIndex in range(BOARD_WIDTH):
            if board[(columnIndex, rowIndex)] == EMPTY_SPACE:
